...
Reuses the roster convention from `DUCKDB_PLAN.md`. Roles, not fixed
individuals — any capable agent may be assigned a role for a given task, but
each in-flight task has exactly one owning agent.

| Role | Responsibility |
| --- | --- |
| **Sol** | Diagnosis, benchmark/measurement design, ambiguity-policy and disambiguation-rule design, gate approval. Sol does not implement production fixes; Sol verifies findings against real code/data and reviews implementations against them. |
| **Terra** | Production implementation in `openNASR/flightplan.py`: parser, procedure resolution, airway resolution, performance/caching. |
| **Luna** | Fixtures, regression tests, the batch-validation utility, benchmark tooling, and documentation. Luna's tests must fail against the current, unfixed behavior and pass once Terra's fix lands. |
...
- [ ] **T0.2 — Sol.** Categorize each of T0.1's failures as: parser error,
  ...
  named explicitly.
- [ ] **T0.3 — Luna.** Add a non-flaky regression sample: a fixed, curated
  ...
  Dependencies: T0.2.
- [ ] **T0.4 — Sol.** Set and document a domestic-only success target once
  ...
  Dependencies: T0.2.
- [x] **T0.5 — Sol. Done (2026-08-17).** Benchmark cold vs. warm
...
- [ ] **T0.6 — Terra.** Keep `flight_plan_path(nasr, flight_plan)` as the
...
- [ ] **T0.7 — Luna.** Document supported input forms in the module
...
- [ ] **T0.8 — Sol.** Define the typed result/error policy precisely:
...
- [ ] **T0.9 — Terra.** Preserve source coordinates and source ordering in
...
- [x] **T1.0 — Sol. Verified (2026-08-17).** `flight_plan_path`'s airway
  ...
  procedure.
- [ ] **T1.1 — Terra.** Fix the bug found in T1.0. In
...
- [ ] **T1.3 — Terra.** Strip only the trailing speed/altitude suffix from
...
- [ ] **T1.4 — Terra.** Recognize airport endpoint context, including FAA
...
- [ ] **T1.5 — Luna.** Add parser tests for mixed spaces/dots, consecutive
...
- [ ] **T2.1 — Terra.** Resolve a bare departure name (e.g. `RUGGD3`,
  ...
  Dependencies: T1.1.
- [ ] **T2.2 — Terra.** Resolve a bare arrival name preceding the
  ...
  Dependencies: T1.1.
- [ ] **T2.3 — Terra.** Resolve `PROCEDURE.TRANSITION` forms such as
  ...
  beyond T2.4's fix.
- [ ] **T2.4 — Terra.** Fix the greedy-dot-merge bug verified 2026-08-17:
  ...
  functions).
- [ ] **T2.5 — Terra.** Use neighboring route tokens to choose the
  ...
  Dependencies: T2.4.
- [ ] **T2.6 — Terra.** Support routes that combine a DP, direct segment,
...
- [ ] **T2.7 — Luna.** Test a current-cycle valid route for each of: bare
...
- [ ] **T3.1 — Terra.** Fix the airway-designation matching bug verified
...
- [ ] **T3.2 — Terra.** Resolve airway paths only between confirmed
...
- [ ] **T3.3 — Luna.** Clarify what "airway designation" means for this
  ...
  text.
  Dependencies: T3.1.
- [ ] **T3.4 — Terra.** Use context to disambiguate duplicate identifiers
...
- [ ] **T3.5 — Terra.** Prefer a non-`VOT` navaid when a bare `NAV_ID`
...
- [ ] **T3.6 — Terra.** Validate that an airway endpoint exists on the
...
- [ ] **T3.7 — Luna.** Add regression tests for forward/reverse routes,
...
- [ ] **T4.1 — Terra.** Vectorize `_WaypointResolver.__init__`
  ...
  before starting).
- [ ] **T4.2 — Terra.** Vectorize `_airway_vertices`'s `AWY_BASE` lookup
...
- [ ] **T4.3 — Terra.** Add an explicit route resolver/session object
...
- [ ] **T4.5 — Luna.** Benchmark CSV and DuckDB storage with the same
  ...
  Dependencies: T4.3.
- [ ] **T4.6 — Luna.** For every procedure benchmark in T4.5, report the
  ...
  Dependencies: T4.5.
- [ ] **T4.7 — Sol.** Confirm no raw SQL API is introduced for route
...
- [ ] **T5.1 — Luna.** Add a maintained validation command or test utility
  ...
  run).
- [ ] **T5.2 — Terra.** Report failure type, token, token position,
...
- [ ] **T5.3 — Luna.** Run the T0.1 deterministic sample (or T0.3's fixed
...
- [ ] **T5.4 — Luna.** Maintain a small, cycle-pinned procedure evaluation
...
- [ ] **T5.5 — Luna.** Add an opt-in full-file validation mode (an
...
- [ ] **T6.1 — Terra.** Detect and label records outside domestic NASR
...
- [ ] **T6.2 — Sol.** Decide whether coordinate fixes should be parsed
...
- [ ] **T6.3 — Terra.** If T6.2 decides to parse coordinate fixes locally,
...
- [ ] **T6.4 — Sol.** Evaluate optional data providers for international
  ...
  (see Release Gates).
- [ ] **T6.5 — Terra.** Keep this expansion opt-in so the domestic
