...
**Gate 2 — APPROVED (2026-08-17, production `3fd11e7`).** The T2.7-focused
matrix passes (25 tests), and the canonical sample improves from 38/100 to
60/100 overall and from 36/84 to 58/84 on the fixed domestic denominator.
Twenty-two former failures now return paths with zero regressions among
former no-exception rows. Procedure-resolution errors shrink from 40 to 8
and parser errors from 2 to 0. Airway failures grow from 7 to 17 because
successful procedure expansion exposes later airway defects, including the
known T3.1 `Q`/`Y` designation bug; those outcomes remain failures and are
not credited to Phase 2. This approval supersedes the failed `4960111`
review while preserving it as audit evidence in the linked note and Decision
log. Full counts are in
[`docs/route_path_baseline_2026-05-14.md`](docs/route_path_baseline_2026-05-14.md#gate-2-approval-rerun-2026-08-17).
