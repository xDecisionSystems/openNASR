...
def _validate_latlon(latitudes, longitudes):
    ...
    if np_any(~isfinite(longitudes)) or np_any(
...
def ll2xy(lats, lons, latc=None, lonc=None, llc=None):
    ...
    y = (
        earthRad
        * (cos(latc) * sin(lats) - sin(latc) * cos(lats) * cos(lons - lonc))
        / cosc
    )
    dist = sqrt(x**2 + y**2)
...
def xy2ll(x, y, latc=None, lonc=None, llc=None):
    ...
    longitudes_rad = lonc + arctan2(
        x * sin_c, safe_rho * cos(latc) * cos_c - y * sin(latc) * sin_c
    )
